document.addEventListener('DOMContentLoaded', () => {
  ...
  // Key ကို ဤ Device နှင့် ချိတ်ဆက်ခြင်း (Key + Device ID တွဲသိမ်း)
  ...
  function isKeyValidForThisDevice(key) {
    ...
    if (!boundData) return true; // မသုံးရသေးပါ

    try {
      const { key: boundKey, deviceId: boundDeviceId } = JSON.parse(boundData);
      const currentDeviceId = getDeviceId();

      // Key နှင့် Device ID နှစ်ခုလုံး တူမှသာ သုံးခွင့်ပြုမည်
      return boundKey === key && boundDeviceId === currentDeviceId;
    } catch (error) {
      // JSON ပျက်နေပါက ပြန်လည်သတ်မှတ်
  ...
  logoutBtn.addEventListener('click', () => {
    ...
    // Logout လုပ်တဲ့အခါ Device Binding ကို မဖျက်ဘူး (ပြန်သုံးလို့ရအောင်)
    showLoginForm();
  });
